...
class DataServiceAPIs {                                                             // .(10105.02.1 RAM Make it work for any table)
/*
 async showAll( ) { ... }
...
// ----- -------- = -- -------------- : -----------------------------------------
       ...
       constructor( aURL, aTable ) {                                                // .(10105.02.2 RAM Add aTable)

         this.URL   =  aURL ? aURL : 'http://localhost:55301'
         this.Table =  aTable                                                       // .(10105.02.3)
         this.Token = ''
...
// async function showAll( ) { ... }
 async showAll( ) {
  ...
  return axios
        .get(   this.URL + `/api/${this.Table}` )                                   // .(10105.02.4)
...
async doAPI( aMethod, aRoute, pBody ) {
      ...
      if ( ! this.Token ) {
//    if ( ! aRoute.match(        /Members|all/i  ) ) { ... }                           //#.(10105.02.5)
      if ( ! aRoute.match( `/${this.Table}|all/i` ) ) {                                 // .(10105.02.5)
...
// ----- -- ------ = -- -------------- : -----------------------------------------------------
 ...
 //      exports = Members  The requested module './Members.js' is a CommonJS module, which may not support all module.exports as named exports.
  module.exports = DataServiceAPIs
