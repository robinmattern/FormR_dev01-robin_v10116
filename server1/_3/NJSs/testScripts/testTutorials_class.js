...
async doAPI( aMethod, aRoute, pBody ) {
      ...
      if ( pBody && pBody.id    ) { this.ID       = pBody.id;    delete pBody.id    }   // .(01027.01.1)

      if ( ! this.Token ) {
      if ( ! aRoute.match( /Tutorials|all/i ) ) {
...
/*       doit = async ( ) => {      // workie
         ...
         pTuts     =  new Tutorials( 'http://localhost:50315' )
         pData     =  await pTuts.showAll( ); console.log( "pTutorials:", pData )
         }
...
/*
       ( async ( ) => {              // workie
         pTuts     =  new Tutorials( 'http://localhost:50315' )
//       pData     =  await pTuts.showAll(  ); console.log( "pTutorials:", pData )
         console.log( "pTutorials:", await pTuts.testUser( '/api/test/user', 'mod', '12345678') )
         } )()
*/
// ----- -- ------ = -- -------------- : -----------------------------------------------------

 //      exports = Tutorials  The requested module './tutorials.js' is a CommonJS module, which may not support all module.exports as named exports.
