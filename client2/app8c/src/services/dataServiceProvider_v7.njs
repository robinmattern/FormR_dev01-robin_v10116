...
/*
            ...
            getOne              GET    http://my.api.url/posts/1
            getMany             GET    http://my.api.url/posts?filter={"id":[ 1,2,3 ]}
            getManyReference    GET    http://my.api.url/posts?filter={"author_id":1}
            create              POST   http://my.api.url/posts
            update              PUT    http://my.api.url/posts/1
            updateMany Multiple PUT    http://my.api.url/posts/1
            deleteMany Multiple DELETE http://my.api.url/posts/1
            delete              DELETE http://my.api.url/posts/1

            getList(           'posts'   , { filter: {  author_id: 1 }, sort: { field: 'title', order:  'ASC'  },   pagination: { page: 1, perPage: 5 } } );
            getOne(            'posts'   , { id:    1                                                                                                   } );
            getMany(           'posts'   , { ids: [ 1,2,3 ]                                                                                             } );
            getManyReference(  'comments', { id:    1,                  sort: { field: 'title', order:  'DESC' },       target: 'post_id'               } );
            update(            'posts'   , { id:    1,                  data: { title: "hello, world" }, previousData: { title: "previous title" }      } );
            updateMany(        'posts'   , { ids: [ 1,2 ],              data: { views:  0 }                                                             } );
            create(            'posts'   , {                            data: { title: "hello, world" }                                                 } );
            delete(            'posts'   , { id:    1,                                                   previousData: { title: "hello, world"   }      } );
            deleteMany(        'posts'   , { ids: [ 1,2 ]                                                                                               } );
...
//      return  httpClient( url, { method: 'POST',   data: JSON.stringify( params.data ) } ).then( function( response ) { return  yDebug( response )                                  } )
        }
